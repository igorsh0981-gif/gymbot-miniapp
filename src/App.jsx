...
function FoodVisionScreen({tgId,onBack,mealType}){
  ...
  async function saveToLog(){
    ...
    try{
      ...
      await fetch(`${API}/nutrition/${tgId}`,{method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({meal_name:result.dish_name,kcal:result.calories,
          protein:result.protein_g,fat:result.fat_g,carb:result.carbs_g,
...
function NutritionScreen({tgId,onBack,onNav}){
  ...
  async function recalcAndUpdateLog(logId){
    ...
    try{
      ...
      await fetch(`${API}/nutrition/${tgId}/${logId}`,{method:"PUT",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({meal_name:r.dish_name,kcal:r.calories,protein:r.protein_g,fat:r.fat_g,carb:r.carbs_g})});
      setEditingLog(null);setEditLogText("");setData(null);load();
    }catch{}finally{setEditLogBusy(false);}
  }
