...
// ─── CATALOG ──────────────────────────────────────────────────────────────────
function GroupTile({g,active,onClick}){
  return (
    <div onClick={onClick} style={{
      background:active?C.accent:C.card, border:`0.5px solid ${active?C.accent:C.border}`,
      borderRadius:12, padding:"14px 6px", textAlign:"center", cursor:"pointer",
      display:"flex",flexDirection:"column",alignItems:"center",gap:6,
    }}>
      <div style={{fontSize:26}}>{g.emoji}</div>
      <div style={{fontSize:10,fontFamily:"monospace",fontWeight:700,color:active?C.bg:C.text,lineHeight:1.2}}>{(tGroup(g)||g.name).toUpperCase()}</div>
    </div>
  );
}
...
function CatalogScreen({exercises,muscleGroups}){
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    <div style={{height:12}}/>
    <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={t("search_placeholder")} style={{width:"100%",background:C.card,border:`0.5px solid ${C.border}`,borderRadius:8,padding:"10px 14px",color:C.text,fontSize:13,fontFamily:"monospace",letterSpacing:1,boxSizing:"border-box",marginBottom:14,outline:"none"}}/>
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:16}}>
      <GroupTile g={{emoji:"🌐",name:"ВСЕ"}} active={!activeGroup} onClick={()=>setActiveGroup(null)}/>
      {muscleGroups?.map(g=><GroupTile key={g.id} g={g} active={activeGroup===g.id} onClick={()=>setActiveGroup(g.id===activeGroup?null:g.id)}/>)}
    </div>
