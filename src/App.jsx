...
function tEquip(eq) {
  ...
  // Try partial match
  for (const [ru, trans] of Object.entries(map)) {
    if (eq.includes(ru)) return eq.replace(ru, trans[lang] || ru);
  }
  return eq;
}
...
function WorkoutDetailScreen({workoutId,tgId,onBack}){
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:16}}>
      {[{l:"ПОДХОДОВ",v:totalSets},{l:"ТОННАЖ",v:`${Math.round((data.total_volume||0)/1000*10)/10} т`},{l:"СТАТУС",v:data.status==="finished"?"✓":data.status||"—"}].map((s,i)=><Card key={i} style={{textAlign:"center"}}><div style={{fontSize:9,color:C.muted,fontFamily:"monospace",marginBottom:4}}>{tSport(s)||s.l}</div><Mono size={14}>{s.v}</Mono></Card>)}
    </div>
  ...
  if(step===STEP.FN)return(
    <div style={{padding:"16px 16px 100px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:16}}>
        ...
        ].map((s,i)=><Card key={i} style={{textAlign:"center"}}><div style={{fontSize:9,color:C.muted,fontFamily:"monospace",marginBottom:4}}>{tSport(s)||s.l}</div><Mono size={18}>{s.v}</Mono></Card>)}
      </div>
  ...
  if(step===1&&viewEx)return(
    <div style={{padding:"16px 16px 100px"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        {[{l:"ПОДХОДОВ",v:viewEx.sets_recommended},{l:"ПОВТОРЕНИЙ",v:viewEx.reps_recommended},{l:"СЛОЖНОСТЬ",v:getDiff(viewEx.difficulty)||viewEx.difficulty}].map((s,i)=>(
          <Card key={i} style={{flex:1,textAlign:"center"}}>
            <div style={{fontSize:9,color:C.muted,fontFamily:"monospace",marginBottom:4}}>{tSport(s)||s.l}</div>
...
function CatalogScreen({exercises,muscleGroups}){
  ...
  if(selected)return <div style={{padding:"16px 16px 100px"}}>
    ...
    <div style={{height:12}}/>
    <div style={{display:"flex",gap:8,marginBottom:14}}>
      {[{l:"ПОДХОДОВ",v:selected.sets_recommended},{l:"ПОВТОРЕНИЙ",v:selected.reps_recommended},{l:"СЛОЖНОСТЬ",v:getDiff(selected.difficulty)||selected.difficulty}].map((s,i)=><Card key={i} style={{flex:1,textAlign:"center"}}><div style={{fontSize:9,color:C.muted,fontFamily:"monospace",marginBottom:4}}>{tSport(s)||s.l}</div><div style={{fontSize:14,fontWeight:700,color:C.accent}}>{s.v||"—"}</div></Card>)}
    </div>
