...
function RestTimer({onClose}){
  ...
  return(
    <div style={{position:"fixed",bottom:80,left:16,right:16,background:C.surface,border:`0.5px solid ${done?C.success:C.accent}`,borderRadius:16,padding:"16px",zIndex:200,boxShadow:"none"}}>
      {/* Прогресс-бар */}
...
Дай короткую оценку (3-4 предложения): что хорошо, что улучшить, совет на следующую тренировку.`;
      const res=await fetch(`${API}/ai/ask`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:q,tg_id:tgId})});
      const d=await res.json();
      setAiReview(d.answer||"");
    }catch{setAiReview("");}
  }
        ...
        {lgReplaceMode?(
          <div style={{marginBottom:16}}>
            ...
            {(exercises||[]).filter(e=>e.muscle_group_id===ex.muscle_group_id&&e.id!==ex.id).slice(0,15).map(alt=>(
              <Card key={alt.id} onClick={()=>{
                setSelExs(prev=>prev.map((e,i)=>i===curIdx?{...alt,order:e.order}:e));
                setSets(prev=>{const n={...prev};if(!n[alt.id])n[alt.id]=[];return n;});
                setLgReplaceMode(false);
              }} style={{marginBottom:8}}>
                <div style={{fontWeight:600,fontSize:14,color:C.text}}>{alt.name}</div>
                <div style={{fontSize:11,color:C.muted,marginTop:2}}>{alt.sets_recommended}×{alt.reps_recommended} · {DIFF_LABEL[alt.difficulty]||alt.difficulty}</div>
              </Card>
            ))}
          </div>
        ):(
          <>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
              ...
              <button onClick={()=>setLgReplaceMode(true)} style={{background:"none",border:`0.5px solid ${C.border}`,borderRadius:8,padding:"5px 9px",color:C.muted,fontSize:11,cursor:"pointer",flexShrink:0,marginLeft:8}}>↔ ЗАМЕНИТЬ</button>
            </div>
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    {!editing?<>
      ...
      {!readOnly&&data?.status==="scheduled"&&<>
        <Btn accent full onClick={()=>onNav&&onNav("active_workout")} style={{marginBottom:10}}>▶ НАЧАТЬ ТРЕНИРОВКУ</Btn>
...
// ─── AI ───────────────────────────────────────────────────────────────────────
function AIScreen({user,tgId,onNav}){
  ...
  async function ask(question){
    ...
    }catch{setAnswer("Ошибка соединения.");}
    ...
    try{
      ...
      const res=await fetch(`${API}/planned/${tgId}`,{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({title:workoutPlan.title||"AI тренировка",planned_datetime:dt,
          exercise_ids:workoutPlan.exercise_ids||[]})});
      ...
      {workoutPlan&&!saved&&(
        <div style={{marginTop:16,padding:"12px 14px",background:"#C8FF0015",border:`0.5px solid ${C.accent}`,borderRadius:10}}>
          ...
          {workoutPlan.exercises?.length>0&&(
            <div style={{fontSize:12,color:C.muted,marginBottom:10}}>{workoutPlan.exercises.slice(0,4).join(" · ")}{workoutPlan.exercises.length>4?` +${workoutPlan.exercises.length-4} ещё`:""}</div>
          )}
...
export default function App(){
  ...
  function render(){
    ...
    if(tab==="ai")return <AIScreen user={user} tgId={tgId} onNav={handleNav}/>;
  }
