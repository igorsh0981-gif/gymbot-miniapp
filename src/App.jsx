...
export default function App() {
  // History-based navigation: state = { tab, screen }
  function getNavState() {
    const s = window.history.state;
    return s && s._gymbot ? s : { _gymbot: true, tab: "menu", screen: null };
  }
  const [navState, setNavState] = useState(getNavState);
  const tab = navState.tab;
  const screen = navState.screen;

  function pushNav(tab, screen) {
    const s = { _gymbot: true, tab, screen };
    window.history.pushState(s, "");
    setNavState(s);
  }
  function replaceNav(tab, screen) {
    const s = { _gymbot: true, tab, screen };
  ...
  useEffect(() => {
    ...
    // Инициализируем history state если пусто
    if (!window.history.state?._gymbot) {
      window.history.replaceState({ _gymbot: true, tab: "menu", screen: null }, "");
    }

    // Слушаем системную кнопку «Назад»
    function onPop(e) {
      const s = e.state;
      if (s && s._gymbot) {
        setNavState(s);
      } else {
        // Если history пуста — возвращаем в меню вместо закрытия
        const home = { _gymbot: true, tab: "menu", screen: null };
  ...
  function renderScreen() {
    ...
    if (screen === "my_workouts_detail") return <MyWorkoutsDetailScreen tgId={tgId} onBack={goBack} />;
    if (screen === "plan_workout") return <PlanWorkoutScreen tgId={tgId} onBack={goBack} />;
    if (screen === "goals") return <GoalsScreen tgId={tgId} onBack={goBack} />;
    if (screen === "reminders") return <RemindersScreen tgId={tgId} onBack={goBack} onNavigate={handleNavigate} />;
