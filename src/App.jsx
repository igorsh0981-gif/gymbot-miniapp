...
function WorkoutDetailScreen({workoutId,tgId,onBack}){
  ...
  useEffect(()=>{
    ...
    fetch(`${API}/workout/${workoutId}?tg_id=${tgId}`)
      ...
      .then(setData)
      .catch(()=>setData({exercises:[],status:"error"}));
  },[workoutId]);

  if(!data)return <div style={{padding:"16px 16px 100px"}}><BackBtn onBack={onBack}/><Loader text="ЗАГРУЗКА"/></div>;
  if(data._nodata)return <div style={{padding:"16px 16px 100px"}}><BackBtn onBack={onBack}/><Card><div style={{color:C.danger,fontFamily:"monospace",fontSize:13}}>Не удалось загрузить тренировку. Войди через Telegram.</div></Card></div>;
...
// ─── ACTIVE WORKOUT ───────────────────────────────────────────────────────────
function ActiveWorkoutScreen({tgId,exercises,muscleGroups,onBack,onFinish}){
  const STEP={SG:0,SE:1,WU:2,OR:3,LG:4,FN:5};
  const [step,setStep]=useState(STEP.SG);
  ...
  async function finish(){
    ...
    if(onFinish)onFinish(); // сброс кэша тренировок
    setStep(STEP.FN);
  }
    ...
    return(
      <div style={{padding:"16px 16px 100px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <Kicker>{curIdx+1} / {selExs.length}</Kicker>
          <button onClick={finish} style={{background:"none",border:`0.5px solid ${C.danger}`,borderRadius:8,padding:"6px 12px",color:C.danger,fontSize:11,fontFamily:"monospace",cursor:"pointer"}}>ЗАВЕРШИТЬ</button>
        </div>
        ...
        <Card accent style={{marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:10,marginTop:10}}>
            ...
            {cType==="distance"&&<>
              ...
              <div style={{flex:1}}><div style={{fontSize:10,color:C.muted,fontFamily:"monospace",marginBottom:6}}>ДИСТАНЦИЯ (КМ)</div><input type="number" step="0.1" value={localSet.distance} onChange={e=>setLocalSet(p=>({...p,distance:e.target.value}))} placeholder="0" style={{background:"none",border:"none",color:C.accent,fontSize:26,fontFamily:"monospace",fontWeight:700,width:"100%",outline:"none"}}/></div>
            </>}
          </div>
        </Card>
        <Btn accent full onClick={doLog} style={{marginBottom:10}}>+ ЗАПИСАТЬ ПОДХОД</Btn>
        ...
        <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4}}>
          {selExs.map((e,i)=>(
            <button key={e.id} onClick={()=>setCurIdx(i)} style={{flexShrink:0,padding:"4px 10px",borderRadius:16,background:i===curIdx?C.accent:C.card,border:`0.5px solid ${i===curIdx?C.accent:C.border}`,color:i===curIdx?C.bg:C.muted,fontSize:11,fontFamily:"monospace",cursor:"pointer"}}>{i+1}</button>
          ))}
        </div>
      </div>
    );
...
// ─── PLANNED DETAIL ───────────────────────────────────────────────────────────
function PlannedDetailScreen({pwId,tgId,onBack,exercises,readOnly=false}){
  const [data,setData]=useState(null);
  const [editing,setEditing]=useState(false);
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    {!editing?<>
      ...
      {!readOnly&&data?.status==="scheduled"&&<Btn full onClick={()=>setEditing(true)} style={{marginBottom:16}}>✏ ИЗМЕНИТЬ ДАТУ/СОСТАВ</Btn>}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,marginTop:16,marginBottom:16}}>
        ...
        <Card><div style={{fontSize:11,color:C.muted,fontFamily:"monospace",marginBottom:8}}>ДАТА</div><input type="date" value={form.date} onChange={e=>setForm(p=>({...p,date:e.target.value}))} style={{colorScheme:"dark",background:"none",border:"none",color:C.accent,fontSize:18,fontFamily:"monospace",fontWeight:700,outline:"none",width:"100%"}}/></Card>
        <Card><div style={{fontSize:11,color:C.muted,fontFamily:"monospace",marginBottom:8}}>ВРЕМЯ</div><input type="time" value={form.time} onChange={e=>setForm(p=>({...p,time:e.target.value}))} style={{colorScheme:"dark",background:"none",border:"none",color:C.accent,fontSize:18,fontFamily:"monospace",fontWeight:700,outline:"none",width:"100%"}}/></Card>
      </div>
...
function PlanWorkoutScreen({tgId,exercises,muscleGroups,onBack}){
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
      ...
      <Card><div style={{fontSize:11,color:C.muted,fontFamily:"monospace",marginBottom:8}}>ДАТА</div><input type="date" value={form.date} min={new Date().toISOString().split("T")[0]} onChange={e=>setForm(p=>({...p,date:e.target.value}))} style={{colorScheme:"dark",background:"none",border:"none",color:C.accent,fontSize:20,fontFamily:"monospace",fontWeight:700,outline:"none",width:"100%"}}/></Card>
      <Card><div style={{fontSize:11,color:C.muted,fontFamily:"monospace",marginBottom:8}}>ВРЕМЯ</div><input type="time" value={form.time} onChange={e=>setForm(p=>({...p,time:e.target.value}))} style={{colorScheme:"dark",background:"none",border:"none",color:C.accent,fontSize:20,fontFamily:"monospace",fontWeight:700,outline:"none",width:"100%"}}/></Card>
    </div>
...
function CheckinScreen({onBack,tgId}){
  ...
  async function send(){
    setSaving(true);
    const body={...form,weight:parseFloat(form.weight)||null};
    try{
      if(tgId){
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    {sent
      ...
      :<>
        <Card style={{marginBottom:12}}>
          <div style={{fontSize:12,color:C.muted,fontFamily:"monospace",marginBottom:8}}>ВЕС СЕГОДНЯ (КГ)</div>
          <input type="number" step="0.1" value={form.weight} onChange={e=>setForm(p=>({...p,weight:e.target.value}))} placeholder="0.0"
            style={{background:"none",border:"none",color:C.accent,fontSize:28,fontFamily:"monospace",fontWeight:700,width:"100%",outline:"none"}}/>
        </Card>
        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
          {sliders.map(s=>(
            <Card key={s.key}>
...
export default function App(){
  ...
  function render(){
    ...
    if(screen==="active_workout")return <ActiveWorkoutScreen tgId={tgId} exercises={exercises} muscleGroups={muscleGroups} onBack={goBack} onFinish={()=>setWorkouts(null)}/>;
    ...
    if(screen==="planned_detail")return <PlannedDetailScreen pwId={p.pwId} tgId={tgId} onBack={goBack} exercises={exercises} readOnly={p.readOnly||false}/>;
