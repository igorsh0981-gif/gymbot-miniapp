...
function MenuScreen({user,onNav,activeWorkout=false}){
  ...
  useState(()=>{
    if(tgId){
      // Калории за сегодня (еда)
      fetch(`${API}/nutrition/${tgId}`).then(r=>r.json()).then(d=>{
        const eaten=Math.round((d.calories_today||0));
        // Потраченные калории (спорт за сегодня)
        fetch(`${API}/sport/${tgId}`).then(r=>r.json()).then(s=>{
          const todayStr=new Date().toISOString().slice(0,10);
          const burned=Math.round((s.sessions||[]).filter(x=>x.date&&x.date.slice(0,10)===todayStr).reduce((a,x)=>a+(x.calories_burned||0),0));
...
function OnboardingScreen({tgId,tgUser,onComplete}){
  ...
  async function finish(){
    ...
    try{
      const body={
        telegram_id:tgId,
        first_name:form.first_name.trim()||tgUser?.first_name||"Атлет",
      ...
      onComplete();
    }catch{setErr("Ошибка. Попробуй ещё раз.");}
    finally{setSaving(false);}
  }
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,padding:"32px 20px 24px",display:"flex",flexDirection:"column"}}>
        ...
        {step===1&&<>
          ...
          <div style={{fontSize:13,color:C.muted,marginBottom:28}}>Введи имя, которое будет в приложении</div>
          <input
            value={form.first_name}
            onChange={e=>upd("first_name",e.target.value)}
            placeholder={tgUser?.first_name||"Твоё имя"}
            maxLength={40}
            autoFocus
            style={{width:"100%",background:C.card,border:`1px solid ${form.first_name.trim()?C.accent:C.border}`,borderRadius:10,padding:"14px 16px",color:C.text,fontSize:18,outline:"none",fontFamily:"inherit",transition:"border-color 0.2s"}}
          />
          ...
          <div style={{flex:1}}/>
          <Btn accent full
            disabled={!form.first_name.trim()&&!tgUser?.first_name}
            onClick={()=>{if(form.first_name.trim()||tgUser?.first_name)setStep(2);}}
            style={{marginTop:24}}>
            ДАЛЕЕ →
          </Btn>
        </>}
        ...
        {step===2&&<>
          ...
          <div style={{fontSize:13,color:C.muted,marginBottom:24}}>AI-тренер будет подстраивать советы под неё</div>
          <div style={{display:"flex",flexDirection:"column",gap:10,flex:1}}>
            {goals.map(g=>(
              <div key={g.v} onClick={()=>upd("desired_result",g.v)}
                ...
                <span style={{fontSize:15,fontWeight:600,color:form.desired_result===g.v?C.accent:C.text}}>{g.label}</span>
          ...
          <div style={{display:"flex",gap:10,marginTop:24}}>
            ...
            <Btn accent disabled={saving} onClick={finish} style={{flex:2}}>
              {saving?"ВХОДИМ...":"НАЧАТЬ 🏋️"}
            </Btn>
          </div>
        </>}
