...
function MenuScreen({user,onNav,activeWorkout=false}){
  ...
  // Приветствие: сначала определяем ситуацию, внутри неё берём один из нескольких вариантов.
  // Вариант привязан к дате (а не Math.random), чтобы текст не прыгал при каждом
  // обновлении экрана в течение одного дня, но менялся день ото дня.
  const greetPool=(()=>{
    const d=user?.days_since_workout;
    if(d!==null&&d!==undefined&&d>=7)return t("greet_away");
    if(d===0)return t("greet_today");
    if(streak>=7)return t("greet_streak");
    return t("greet_normal");
  })();
  const greetText=(()=>{
    const pool=Array.isArray(greetPool)?greetPool:null;
    if(!pool||!pool.length)return t("greeting_back2")||"ТЫ ВЕРНУЛСЯ";
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    <div style={{marginBottom:16}}>
      ...
      <Hero>{greetText}{user?.first_name?`, ${user.first_name.split(" ")[0].toUpperCase()}`:""}</Hero>
