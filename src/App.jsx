...
function FoodVisionScreen({tgId,onBack}){
  ...
  return(
    <div style={{padding:"16px 16px 100px"}}>
      ...
      {!preview?(
        <div style={{marginBottom:16}}>
          <Btn accent full onClick={()=>inputRef.current?.click()} style={{marginBottom:10,fontSize:16,padding:"16px"}}>
            📷 СДЕЛАТЬ ФОТО
          </Btn>
          <Btn full onClick={()=>galRef.current?.click()} style={{fontSize:14}}>
            🖼 Выбрать из галереи
          </Btn>
        </div>
      ):(
