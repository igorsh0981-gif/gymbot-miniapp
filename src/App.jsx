...
function MenuScreen({user,onNav}){
  ...
  const actions=[
    ...
    {label:"📷 Фото → КБЖУ",icon:"📷",s:"food_vision"},
      ...
      {showDatePicker&&(
        <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:C.surface,borderRadius:16,padding:20,width:"100%",maxWidth:360}}>
            ...
            <div style={{marginBottom:12}}>
              <div style={{fontSize:11,color:C.muted,fontFamily:"monospace",marginBottom:6}}>ДАТА</div>
...
// ─── FOOD VISION ─────────────────────────────────────────────────────────────
function FoodVisionScreen({tgId,onBack}){
  ...
  async function saveToLog(){
    ...
    try{
      await fetch(`${API}/food/analyze`,{method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({image_base64:photo,tg_id:tgId,save:true})});
      setSaved(true);
...
function NutritionScreen({tgId,onBack,onNav}){
  ...
  async function deleteLog(logId){
    if(!tgId||!logId)return;
    try{await fetch(`${API}/nutrition/${tgId}/${logId}`,{method:"DELETE"});setData(null);load();}catch{}
  }
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    {!adding
      ?<div style={{display:"flex",gap:8,marginBottom:16}}>
        <Btn accent full onClick={()=>setAdding(true)} style={{flex:2}}>+ ДОБАВИТЬ ПРИЁМ ПИЩИ</Btn>
        <Btn full onClick={()=>onNav&&onNav("food_vision")} style={{flex:1}}>📷 ФОТО</Btn>
      </div>
      :<Card style={{marginBottom:16}}>
          ...
          <Card key={i}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                ...
                {l.id&&<button onClick={()=>deleteLog(l.id)}
                  style={{background:"none",border:`0.5px solid ${C.danger}`,borderRadius:6,
                    padding:"3px 8px",color:C.danger,fontSize:11,cursor:"pointer",flexShrink:0}}>
                  ✕
                </button>}
              </div>
            </div>
...
export default function App(){
  ...
  function render(){
    ...
    if(screen==="food_vision")return <FoodVisionScreen tgId={tgId} onBack={goBack}/>;
