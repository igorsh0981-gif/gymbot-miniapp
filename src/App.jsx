...
function CatalogScreen({exercises,muscleGroups}){
  ...
  if(selected)return <div style={{padding:"16px 16px 100px"}}>
    ...
    <div style={{height:12}}/>
    <div style={{display:"flex",gap:8,marginBottom:14}}>
      {[{l:"ПОДХОДОВ",v:selected.sets_recommended},{l:"ПОВТОРЕНИЙ",v:selected.reps_recommended},{l:"СЛОЖНОСТЬ",v:DIFF_LABEL[selected.difficulty]||selected.difficulty}].map((s,i)=><Card key={i} style={{flex:1,textAlign:"center"}}><div style={{fontSize:9,color:C.muted,fontFamily:"monospace",marginBottom:4}}>{tSport(s)||s.l}</div><div style={{fontSize:14,fontWeight:700,color:C.accent}}>{s.v||"—"}</div></Card>)}
    </div>
