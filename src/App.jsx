...
const GOAL_LABELS={lose_weight:t("goal_lose"),gain_muscle:t("goal_muscle"),gain_strength:"Сила",improve_endurance:t("goal_endurance"),stay_healthy:"Здоровье"};
const LEVEL_LABELS={beginner:"Новичок",intermediate:t("level_intermediate"),advanced:t("level_advanced")};
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    {!editing?<>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
        ...
        <Card><div style={{fontSize:11,color:C.muted,fontFamily:"monospace",marginBottom:8}}>УРОВЕНЬ</div><Sel value={form.fitness_level} onChange={v=>setForm(p=>({...p,fitness_level:v}))} options={[{value:"beginner",label:"Новичок"},{value:"intermediate",label:t("level_intermediate")},{value:"advanced",label:t("level_advanced")}]}/></Card>
        <Card><div style={{fontSize:11,color:C.muted,fontFamily:"monospace",marginBottom:8}}>ЦЕЛЬ</div><Sel value={form.desired_result} onChange={v=>setForm(p=>({...p,desired_result:v}))} options={[{value:"lose_weight",label:t("goal_lose")},{value:"gain_muscle",label:t("goal_muscle")},{value:"gain_strength",label:"Сила"},{value:"improve_endurance",label:t("goal_endurance")},{value:"stay_healthy",label:"Здоровье"}]}/></Card>
        <Card style={{marginBottom:12}}>
  ...
  async function finish(){
    ...
    try{
      ...
      const exList=selExs.map((e,i)=>{
        ...
        return `${i+1}. ${e.name}: ${setsStr||t("no_sets_data")}`;
      }).join("\n");
    ...
    {norms?.norms&&<Card style={{marginBottom:16}}>
      ...
      {showNorms&&<div style={{marginTop:12,paddingTop:12,borderTop:`0.5px solid ${C.border}`}}>
        <div style={{fontSize:11,color:C.muted,marginBottom:10,fontFamily:"monospace"}}>
          Возраст {norms.age}л · {norms.bodyweight}кг · коэф. {norms.age_correction}
        </div>
        {norms.norms.map((n,i)=>{
          ...
          const levelLabel={below_beginner:"Ниже нач.",beginner:t("level_beginner"),intermediate:t("level_intermediate"),advanced:t("level_advanced"),elite:"Элита"}[n.your_level]||"—";
          ...
          return<div key={i} style={{marginBottom:12,paddingBottom:12,borderBottom:i<norms.norms.length-1?`0.5px solid ${C.border}`:"none"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
              ...
              <div style={{textAlign:"right"}}>
                {n.your_max
                  ...
                  :<span style={{fontSize:11,color:C.muted}}>нет данных</span>
                }
              </div>
            </div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:4,fontSize:10,color:C.muted,fontFamily:"monospace"}}>
              <span>Нач: {n.levels.beginner}{n.unit||"кг"}</span>
              <span>Ср: {n.levels.intermediate}{n.unit||"кг"}</span>
              <span>Прод: {n.levels.advanced}{n.unit||"кг"}</span>
              <span>Элита: {n.levels.elite}{n.unit||"кг"}</span>
            </div>
          </div>;
        })}
      </div>}
...
function CatalogScreen({exercises,muscleGroups}){
  ...
  if(selected)return <div style={{padding:"16px 16px 100px"}}>
    ...
    {selected.description&&<Card style={{marginBottom:10}}><Kicker>{t("description_label")}</Kicker><div style={{fontSize:14,color:C.text,lineHeight:1.6,marginTop:6}}>{selected.description}</div></Card>}
    {selected.technique&&<Card style={{marginBottom:10}}>
      <Kicker>{t("technique_label")}</Kicker>
      <div style={{fontSize:14,color:C.text,lineHeight:1.8,marginTop:6,whiteSpace:"pre-wrap"}}>{selected.technique}</div>
    </Card>}
    {selected.equipment&&<Card><Kicker>{t("equipment_label")}</Kicker><div style={{fontSize:14,color:C.text,marginTop:4}}>{selected.equipment}</div></Card>}
  </div>;

  return <div style={{padding:"16px 16px 100px"}}>
    <Kicker>{t("knowledge_base")}</Kicker><Hero>УПРАЖНЕНИЯ</Hero>
    <div style={{height:12}}/>
...
function AIScreen({user,tgId,onNav,exercises=[]}){
  ...
  return <div style={{padding:"16px 16px 160px"}}>
    ...
    {saved&&<div style={{marginBottom:12,background:"#00CC6622",border:`0.5px solid ${C.success}`,borderRadius:10,padding:"10px 14px",fontSize:12,color:C.success,fontFamily:"monospace"}}>✓ ТРЕНИРОВКА СОХРАНЕНА → вкладка Запланированные</div>}
...
function FoodGuideScreen({onBack}){
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    <BackBtn onBack={onBack}/><Kicker>{t("supps_ref")}</Kicker><Hero>КАТАЛОГ ПРОДУКТОВ</Hero>
    <div style={{height:16}}/>
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
...
function CheckinScreen({onBack,tgId}){
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    <div style={{height:8}}/>
    {sent
      ?<Card accent><div style={{textAlign:"center",padding:"20px 0"}}><div style={{fontSize:32,color:C.accent}}>✓</div><div style={{fontFamily:"monospace",fontSize:13,color:C.accent,marginTop:8}}>{t("checkin_saved")}</div><div style={{fontSize:12,color:C.muted,marginTop:6}}>AI тренер получил обновлённые данные</div></div></Card>
      :<>
...
function LanguageScreen({tgId,user,onBack,onUserUpdated}){
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    <BackBtn onBack={onBack}/><Kicker>{t("settings_title")}</Kicker><Hero>ЯЗЫК</Hero><div style={{height:16}}/>
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
...
function RemindersScreen({tgId,onBack,onNav}){
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    {data.reminders.length===0?<Card><div style={{textAlign:"center",padding:"16px 0"}}><div style={{color:C.muted,fontSize:14}}>Нет предстоящих тренировок</div><button onClick={()=>onNav("plan_workout")} style={{marginTop:10,background:"none",border:`0.5px solid ${C.accent}`,borderRadius:8,padding:"8px 16px",color:C.accent,fontSize:12,fontFamily:"monospace",cursor:"pointer"}}>{t("add_plan")}</button></div></Card>:
        ...
        <Card key={i} style={{
          ...
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{textAlign:"right",flexShrink:0}}>
              ...
              <div style={{fontSize:10,color:C.muted}}>балл.</div>
            </div>
          </div>
        </Card>
      ))}
        ...
        <Card key={i} accent style={{marginBottom:8,padding:"12px 16px"}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div>
              ...
              <div style={{fontSize:12,color:C.muted,marginTop:2}}>{a.desc} · +{a.pts} балл.</div>
            </div>
          </div>
        </Card>
      ))}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
        {unlocked.map((a,i)=>(
          <Card key={i} style={{border:`0.5px solid ${C.accent}33`}}>
            ...
            <div style={{fontWeight:600,fontSize:13,color:C.text,marginBottom:3}}>{a.name}</div>
            <div style={{fontSize:11,color:C.muted,lineHeight:1.4,marginBottom:6}}>{a.desc}</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        {locked.map((a,i)=>(
          <Card key={i} style={{opacity:0.5}}>
            ...
            <div style={{fontWeight:600,fontSize:13,color:C.muted,marginBottom:3}}>{a.name}</div>
            <div style={{fontSize:11,color:C.muted,lineHeight:1.4,marginBottom:6}}>{a.desc}</div>
