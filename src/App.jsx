...
function MenuScreen({user,onNav,activeWorkout=false}){
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    {user&&<>
      ...
      <Card onClick={()=>onNav("profile")} style={{marginTop:8,marginBottom:8}}>
  ...
  if(data.length===1){
    ...
    return<Card style={{marginBottom:12}}>
      ...
      <div style={{display:"flex",gap:16,padding:"8px 0"}}>
        <div><div style={{fontSize:9,color:C.muted,fontFamily:"monospace"}}>ДАТА</div><Mono size={13}>{String(d.date).slice(5)}</Mono></div>
...
function OnboardingScreen({tgId,tgUser,onComplete}){
  ...
  function canNext(){
    ...
    if(step===2)return form.age>=10&&form.age<=100;
    if(step===3)return form.weight>=30&&form.weight<=300&&form.height>=100&&form.height<=250;
    if(step===4)return !!form.gender;
    if(step===5)return !!form.fitness_level;
    if(step===6)return !!form.desired_result;
    return true;
  }

  async function finish(){
    if(!tgId){setErr("Ошибка: не удалось получить Telegram ID. Открой приложение через Telegram.");return;}
    setSaving(true);setErr("");
    try{
      const body={
        telegram_id:tgId,
      ...
      onComplete();
    }catch{setErr("Ошибка сохранения. Попробуй ещё раз.");}
    finally{setSaving(false);}
  }

  const progress=(step/STEPS)*100;
  const genders=[{v:"male",icon:"👨",label:"Мужской"},{v:"female",icon:"👩",label:"Женский"}];
  const levels=[
    {v:"beginner",icon:"🌱",label:"Новичок",desc:"менее 1 года"},
    {v:"intermediate",icon:"💪",label:"Средний",desc:"1–3 года"},
    {v:"advanced",icon:"🔥",label:"Продвинутый",desc:"3+ лет"},
  ];
  const goals=[
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",padding:"0 0 40px"}}>
      {/* Header */}
      <div style={{padding:"24px 20px 16px",borderBottom:`1px solid ${C.border}`}}>
        <div style={{fontFamily:"monospace",fontSize:11,color:C.muted,letterSpacing:2,marginBottom:8}}>
          ШАГ {step} / {STEPS}
        </div>
        <div style={{height:4,background:C.border,borderRadius:2}}>
          <div style={{height:4,background:C.accent,borderRadius:2,width:`${progress}%`,transition:"width 0.3s"}}/>
        </div>
      </div>
      ...
      <div style={{flex:1,padding:"24px 20px"}}>
        ...
        {step===1&&<>
          <Kicker>ДОБРО ПОЖАЛОВАТЬ</Kicker>
          <Hero style={{marginBottom:8}}>КАК ТЕБЯ ЗОВУТ?</Hero>
          <div style={{fontSize:13,color:C.muted,marginBottom:24}}>Введи имя, которое будет отображаться в приложении</div>
          <input
            value={form.first_name}
            onChange={e=>upd("first_name",e.target.value)}
            placeholder="Твоё имя"
            maxLength={40}
            style={{width:"100%",background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"14px 16px",color:C.text,fontSize:16,outline:"none",fontFamily:"inherit"}}
          />
        </>}

        {step===2&&<>
          <Kicker>О ТЕБЕ</Kicker>
          <Hero style={{marginBottom:8}}>СКОЛЬКО ЛЕТ?</Hero>
          <div style={{fontSize:13,color:C.muted,marginBottom:24}}>Возраст влияет на расчёт нормы калорий и нагрузки</div>
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            <button onClick={()=>upd("age",Math.max(10,(parseInt(form.age)||18)-1))} style={{width:48,height:48,borderRadius:12,background:C.card,border:`1px solid ${C.border}`,color:C.text,fontSize:24,cursor:"pointer"}}>−</button>
            <input
              type="number" value={form.age} onChange={e=>upd("age",e.target.value)}
              placeholder="25" min={10} max={100}
              style={{flex:1,textAlign:"center",background:C.card,border:`1px solid ${C.accent}`,borderRadius:10,padding:"14px",color:C.text,fontSize:32,fontWeight:700,outline:"none"}}
            />
            <button onClick={()=>upd("age",Math.min(100,(parseInt(form.age)||18)+1))} style={{width:48,height:48,borderRadius:12,background:C.card,border:`1px solid ${C.border}`,color:C.text,fontSize:24,cursor:"pointer"}}>+</button>
          </div>
          <div style={{textAlign:"center",fontSize:12,color:C.muted,marginTop:8,fontFamily:"monospace"}}>лет</div>
        </>}

        {step===3&&<>
          <Kicker>ПАРАМЕТРЫ ТЕЛА</Kicker>
          <Hero style={{marginBottom:8}}>ВЕС И РОСТ</Hero>
          <div style={{fontSize:13,color:C.muted,marginBottom:24}}>Используется для расчёта ИМТ и норм питания</div>
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            <div>
              <div style={{fontFamily:"monospace",fontSize:11,color:C.muted,letterSpacing:1,marginBottom:8}}>ВЕС (КГ)</div>
              <input
                type="number" value={form.weight} onChange={e=>upd("weight",e.target.value)}
                placeholder="70" min={30} max={300} step={0.1}
                style={{width:"100%",background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"14px 16px",color:C.text,fontSize:18,fontWeight:600,outline:"none",textAlign:"center"}}
              />
            </div>
            <div>
              <div style={{fontFamily:"monospace",fontSize:11,color:C.muted,letterSpacing:1,marginBottom:8}}>РОСТ (СМ)</div>
              <input
                type="number" value={form.height} onChange={e=>upd("height",e.target.value)}
        ...
        {step===4&&<>
          <Kicker>О ТЕБЕ</Kicker>
          <Hero style={{marginBottom:8}}>ПОЛ</Hero>
          <div style={{fontSize:13,color:C.muted,marginBottom:24}}>Влияет на расчёт суточных норм</div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {genders.map(g=>(
              <div key={g.v} onClick={()=>upd("gender",g.v)}
                style={{display:"flex",alignItems:"center",gap:16,padding:"16px",borderRadius:12,
                  border:`1px solid ${form.gender===g.v?C.accent:C.border}`,
                  background:form.gender===g.v?"rgba(200,255,0,0.06)":C.card,cursor:"pointer"}}>
                <span style={{fontSize:28}}>{g.icon}</span>
                <span style={{fontSize:16,fontWeight:600,color:form.gender===g.v?C.accent:C.text}}>{g.label}</span>
                {form.gender===g.v&&<span style={{marginLeft:"auto",color:C.accent,fontSize:18}}>✓</span>}
              </div>
            ))}
          </div>
        </>}
        ...
        {step===5&&<>
          <Kicker>ОПЫТ</Kicker>
          <Hero style={{marginBottom:8}}>УРОВЕНЬ ПОДГОТОВКИ</Hero>
          <div style={{fontSize:13,color:C.muted,marginBottom:24}}>Подберём подходящую нагрузку</div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {levels.map(l=>(
              <div key={l.v} onClick={()=>upd("fitness_level",l.v)}
                style={{display:"flex",alignItems:"center",gap:16,padding:"16px",borderRadius:12,
                  border:`1px solid ${form.fitness_level===l.v?C.accent:C.border}`,
                  background:form.fitness_level===l.v?"rgba(200,255,0,0.06)":C.card,cursor:"pointer"}}>
                <span style={{fontSize:26}}>{l.icon}</span>
                <div style={{flex:1}}>
                  <div style={{fontSize:15,fontWeight:600,color:form.fitness_level===l.v?C.accent:C.text}}>{l.label}</div>
                  <div style={{fontSize:12,color:C.muted}}>{l.desc}</div>
                </div>
                {form.fitness_level===l.v&&<span style={{color:C.accent,fontSize:18}}>✓</span>}
              </div>
            ))}
          </div>
        </>}
        ...
        {step===6&&<>
          <Kicker>ФИНАЛ</Kicker>
          <Hero style={{marginBottom:8}}>ТВОЯ ЦЕЛЬ</Hero>
          <div style={{fontSize:13,color:C.muted,marginBottom:24}}>AI-тренер будет строить план под неё</div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {goals.map(g=>(
              <div key={g.v} onClick={()=>upd("desired_result",g.v)}
                style={{display:"flex",alignItems:"center",gap:16,padding:"14px 16px",borderRadius:12,
                  border:`1px solid ${form.desired_result===g.v?C.accent:C.border}`,
                  background:form.desired_result===g.v?"rgba(200,255,0,0.06)":C.card,cursor:"pointer"}}>
                <span style={{fontSize:22}}>{g.icon}</span>
                <span style={{fontSize:14,fontWeight:600,color:form.desired_result===g.v?C.accent:C.text}}>{g.label}</span>
                {form.desired_result===g.v&&<span style={{marginLeft:"auto",color:C.accent,fontSize:18}}>✓</span>}
              </div>
            ))}
          </div>
        </>}

        {err&&<div style={{marginTop:16,padding:"10px 14px",background:"rgba(255,68,68,0.1)",border:`1px solid ${C.danger}`,borderRadius:8,color:C.danger,fontSize:13,fontFamily:"monospace"}}>{err}</div>}
      </div>

      {/* Footer buttons */}
      <div style={{padding:"0 20px",display:"flex",gap:10}}>
        {step>1&&<Btn onClick={()=>setStep(s=>s-1)} style={{flex:1}}>НАЗАД</Btn>}
        {step<STEPS
          ?<Btn accent full={step===1} disabled={!canNext()} onClick={()=>canNext()&&setStep(s=>s+1)} style={{flex:2}}>ДАЛЕЕ →</Btn>
          :<Btn accent full disabled={!canNext()||saving} onClick={finish} style={{flex:2}}>{saving?"СОХРАНЯЕМ...":"НАЧАТЬ 🏋️"}</Btn>
        }
      </div>
    </div>
  );
...
export default function App(){
  ...
  function render(){
    ...
    if(tab==="menu")return <MenuScreen key={lang} user={user} onNav={handleNav} activeWorkout={!!localStorage.getItem("gymbot_active_workout")}/>;
