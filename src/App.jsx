...
function RestTimer({onClose,autoStart=null}){
...
function ActiveWorkoutScreen({tgId,exercises,muscleGroups,onBack,onFinish,onFinishNav=null,onNav=null,preselectedExIds=[],preselectedGroupIds=[],plannedWorkoutId=null,exerciseTips={}}){
  ...
  async function doLog(){
    ...
    if(rs>0){setTimerAuto(rs);setShowTimer(true);}
  }
  async function finish(){
  ...
  if(step===STEP.LG){
    ...
    return(
      <div style={{padding:"16px 16px 100px"}}>
        ...
        {/* Таймер отдыха */}
        {showTimer&&<RestTimer autoStart={timerAuto} onClose={()=>{setShowTimer(false);setTimerAuto(null);}}/>}
      </div>
    );
  }
  return null;
