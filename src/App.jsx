...
// ── FoodSearchScreen ─────────────────────────────────────────────────────────
function FoodSearchScreen({tgId,onBack,mealType}){
  const CATS=[
    {v:"",l:"Все"},
  ...
  async function logProduct(){
    ...
    try{
      ...
      if(r.ok){setSaved(true);setTimeout(()=>onBack(),1200);}
    }catch{}finally{setSaving(false);}
  }
...
export default function App(){
  ...
  function render(){
    ...
    if(screen==="food_search")return <FoodSearchScreen tgId={tgId} onBack={goBack} mealType={p.meal_type}/>;
