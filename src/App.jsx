...
function FoodVisionScreen({tgId,onBack}){
  ...
  return(
    <div style={{padding:"16px 16px 100px"}}>
      ...
      {/* Один input — система сама предлагает камеру или галерею */}
      <input ref={inputRef} type="file" accept="image/*"
        onChange={e=>handleFile(e.target.files[0])} style={{display:"none"}}/>
      <input ref={galRef} type="file" accept="image/*" capture="environment"
        onChange={e=>handleFile(e.target.files[0])} style={{display:"none"}}/>
      {!preview?(
        <div style={{marginBottom:16}}>
          <Btn accent full onClick={()=>galRef.current?.click()}
            style={{marginBottom:10,fontSize:16,padding:"18px"}}>
            📷 СДЕЛАТЬ ФОТО СЕЙЧАС
          </Btn>
          <Btn full onClick={()=>inputRef.current?.click()} style={{fontSize:14}}>
            🖼 Выбрать из галереи
          </Btn>
        </div>
      ):(
