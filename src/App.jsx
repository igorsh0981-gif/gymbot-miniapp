...
function WorkoutHistoryScreen({workouts,onNav,tgId}){
  const [activeTab,setActiveTab]=useState("history"); // "history" | "planned"
  ...
  return <div style={{padding:"16px 16px 100px"}}>
    ...
    <div style={{display:"flex",gap:0,marginBottom:16,border:`0.5px solid ${C.border}`,borderRadius:10,overflow:"hidden"}}>
      {[{id:"history",label:"📋 ИСТОРИЯ"},{id:"planned",label:"📅 ПЛАН"}].map(t=>(
...
function ActiveWorkoutScreen({tgId,exercises,muscleGroups,onBack,onFinish}){
  ...
  const filteredEx=allEx.filter(e=>(!selGroup||e.muscle_group_id===selGroup)&&(!exSearch||e.name.toLowerCase().includes(exSearch.toLowerCase())));
  ...
  if(step===STEP.SG)return(
    <div style={{padding:"16px 16px 100px"}}>
      <BackBtn onBack={onBack}/>
      <Kicker>ШАГ 1 / 4</Kicker><Hero>ГРУППА МЫШЦ</Hero><div style={{height:12}}/>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {muscleGroups?.map(g=>(
          <Card key={g.id} onClick={()=>{setSelGroup(g.id);setExSearch("");setStep(STEP.SE);}}>
        ...
        <Card onClick={()=>{setSelGroup(null);setExSearch("");setStep(STEP.SE);}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
